"""Automated objective acoustic comparison between Rust Native Engine and authentic Steinway Model B recordings."""
...
RUST_BIN = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "target", "release", "physics-piano-rs"))

def compare_rust_note(sample_path: str, note_name: str, target_f0: float, velocity: float = 0.75):
    print(f"\n=======================================================")
    print(f"[*] [Rust Native Engine] Comparing Note: {note_name} against {os.path.basename(sample_path)}")
    print(f"=======================================================")
    ...
    # 2. Synthesize note with Rust physical engine
    duration_sec = 2.5
    tmp_out = f"/tmp/rust_eval_{note_name}.wav"
    cmd = [RUST_BIN, "render", note_name, str(duration_sec), tmp_out, str(velocity), "--sustain"]
    subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    synth_sr, synth_audio = wavfile.read(tmp_out)
    if synth_audio.ndim > 1:
        synth_audio = synth_audio.mean(axis=1)
    synth_audio = synth_audio.astype(np.float64) / 32768.0
    synth_max = np.max(np.abs(synth_audio))
    if synth_max > 0:
        synth_audio /= synth_max

    # Resample if sample rates differ (Rust 48000 Hz vs Steinway 44100 Hz)
    if synth_sr != ref_sr:
        from scipy import signal
        num_target = int(len(synth_audio) * float(ref_sr) / float(synth_sr))
        synth_audio = signal.resample(synth_audio, num_target)
    ...
    print(f"    - Objective Diff Grade(ODG): {peaq_res['odg']:.2f} (Scale: 0 Imperceptible, -1 Perceptible not annoying, -2 Slightly annoying)")
    ...
    print(f"    - Rust Physical Engine:     {tr_synth['rise_time_ms']:.2f} ms")
    print(f"    - Steinway B Reference:      {tr_ref['rise_time_ms']:.2f} ms")

    print(f"[4] Inharmonicity B Coefficient:")
    print(f"    - Rust Physical Engine:     {b_synth['fitted_b']:.6e} (Cent Std: {b_synth['cent_std']:.2f} cents)")
    print(f"    - Steinway B Reference:      {b_ref['fitted_b']:.6e} (Cent Std: {b_ref['cent_std']:.2f} cents)")

if __name__ == "__main__":
    compare_rust_note("steinway_b_samples/Piano.mf.A4.wav", "A4", 440.0, velocity=0.75)
    compare_rust_note("steinway_b_samples/Piano.mf.C4.wav", "C4", 261.63, velocity=0.75)
    compare_rust_note("steinway_b_samples/Piano.mf.C2.wav", "C2", 65.41, velocity=0.75)
